...
func (s *HandlerTestSuite) TestPubPartDetail() {
	...
	err := s.db.WithContext(context.Background()).Create(&res).Error
	require.NoError(t, err)
	err = s.db.WithContext(context.Background()).Create(&que).Error
	require.NoError(t, err)
	...
	req, err := http.NewRequest(http.MethodPost,
		"/question/detail", iox.NewJSONReader(web.Qid{
			Qid: 1041,
		}))
	req.Header.Set("content-type", "application/json")
	...
	assert.Equal(t, web.Question{
		...
		Basic: web.AnswerElement{
			Id:        6111,
			Content:   `<p>在微服务架构中，服务实例的动态变化很常见，比如服务实例因为扩容、缩容或者故障而上下线。为了保障系统的稳定性和高可用性，我们需要一套完善的机制来处理这些变化，主要从以下几个方面入手：</p>`,
			Guidance:  `心跳检测；健康检查；负载均衡；一致性哈希；重试；熔断；限流；降级；扩容；缩容；平滑处理；`,
			Shorthand: `注册中心实时监测，客户端做好容错，扩容要平滑；`,
		},
		...
		Advanced: web.AnswerElement{
			Id:        6113,
			Content:   `<p>而在大规模分布式系统下，如果服务节点动态变化频繁，那么会给注册中心带来庞大的压力。</p>`,
			Guidance:  "CAP；",
			Highlight: "节点变化在大规模集群下对注册中心的影响",
			Shorthand: "注册中心选AP不选CP；",
		},
